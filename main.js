...
(function($) {
    ...
    async function onStory(isDownload,isForce){
        if(isDownload){
            ...
            if($('body > div section:visible video[playsinline]').length > 0){
                ...
                let srcset = $('body > div section:visible img[referrerpolicy][class]:not([alt]), body > div section:visible img[crossorigin][class]:not([alt])').attr('srcset')?.split(',')[0]?.split(' ')[0];
                let link = (srcset)?srcset:$('body > div section:visible img[referrerpolicy][class]:not([alt]), body > div section:visible img[crossorigin][class]:not([alt])').attr('src');
    ...
    function onReadyMyDW(NoDialog){
        ...
        if(NoDialog == false){
            var repeat = setInterval(() => {
                ...
                if($('article[data-snig="canDownload"], section:visible > main > div > div.xdt5ytf > div[data-snig="canDownload"]').length > 0) clearInterval(repeat);
    ...
    function createDownloadButton(){
        ...
        $('article, section:visible > main > div > div.xdt5ytf').each(function(index){
